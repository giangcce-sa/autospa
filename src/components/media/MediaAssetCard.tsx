...
export function MediaAssetCard({
    ...
    <>
      ...
      <div className="space-y-2 p-3">
        <div className="flex min-w-0 items-start justify-between gap-2">
          <h3 className="line-clamp-2 text-sm font-semibold leading-5 text-[var(--text)]">{title}</h3>
          {badges && <div className="flex shrink-0 flex-wrap justify-end gap-1">{badges}</div>}
        </div>
        {description && <p className="line-clamp-2 text-xs leading-5 text-[var(--text-secondary)]">{description}</p>}
        {metadata && <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-[11px] text-[var(--text-muted)]">{metadata}</div>}
  ...
  const className = cn(
    "overflow-hidden rounded-xl border bg-[var(--bg-card)] text-left shadow-[var(--shadow-sm)] transition-[border-color,box-shadow,transform] motion-reduce:transition-none",
    onSelect && "hover:-translate-y-0.5 hover:shadow-[var(--shadow-md)] motion-reduce:hover:translate-y-0",
    selected ? "border-[var(--accent)] ring-2 ring-[var(--accent-light)]" : "border-[var(--border)]",
  );
  ...
  if (actions) {
    return (
      <article className={cn(className, "relative")}>
        <button type="button" onClick={onSelect} aria-pressed={selected} aria-label={title} className="absolute inset-0 z-10 rounded-xl focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent)]" />
