...
function VideoOverview({ projects, providers, onSelect }: { projects: ProjectSummary[]; providers: ProviderStatus | null; onSelect: (id: string) => void }) {
  ...
  return <div className="space-y-6">
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
      {[ ["Dự án", projects.length], ["Job đang chạy", activeJobs], ["Chờ duyệt", readyForReview], ["Đã duyệt đúng revision", approved] ].map(([label, value]) => <div key={String(label)} className="rounded-lg border border-[var(--border)] bg-[var(--bg-card)] p-4"><p className="text-xs text-[var(--text-muted)]">{label}</p><p className="mt-1 text-2xl font-bold text-[var(--text)]">{value}</p></div>)}
    </div>
...
function VideoJobs({ projects, project, onSelect, onRefresh }: { projects: ProjectSummary[]; project: Project | null; onSelect: (id: string) => void; onRefresh: (id: string) => Promise<void> }) {
  ...
  return <div className="space-y-4"><div className="flex items-start justify-between gap-3"><div><button type="button" className="text-xs font-semibold text-[var(--accent)]" onClick={() => onSelect("")}>Quay lại danh sách dự án</button><h2 className="mt-2 text-lg font-bold">{project.name}</h2><p className="text-xs text-[var(--text-muted)]">{project.jobs.length} công việc đã persist</p></div><Button variant="secondary" size="sm" onClick={() => onRefresh(project.id)}><ArrowClockwise size={14} />Làm mới</Button></div><div className="divide-y rounded-lg border border-[var(--border)] bg-[var(--bg-card)]">{project.jobs.length === 0 ? <p className="p-8 text-center text-sm text-[var(--text-muted)]">Dự án chưa có công việc.</p> : project.jobs.map((job) => <article key={job.id} className="grid gap-3 p-4 sm:grid-cols-[minmax(0,1fr)_auto]"><div><div className="flex flex-wrap items-center gap-2"><h3 className="font-semibold">{job.type}</h3><Status value={job.status} /></div><p className="mt-1 text-xs text-[var(--text-muted)]">{job.provider} · Lần thử {job.attempt ?? 0}</p>{job.error && <p className="mt-2 text-xs text-[var(--danger)]">{job.error}</p>}</div><div className="text-right"><p className="text-lg font-bold tabular-nums">{job.progress}%</p><p className="text-[11px] text-[var(--text-muted)]">Tiến độ persist</p></div></article>)}</div></div>;
}

function VideoProjectReadOnly({ project, onBack }: { project: Project; onBack: () => void }) {
  return <div className="space-y-4"><button type="button" className="text-xs font-semibold text-[var(--accent)]" onClick={onBack}>Quay lại danh sách dự án</button><div><h2 className="text-lg font-bold">{project.name}</h2><p className="text-xs text-[var(--text-muted)]">{project.platform} · {project.aspectRatio} · {project.durationSec}s</p></div><div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_18rem]"><div className="space-y-2 rounded-lg border border-[var(--border)] bg-[var(--bg-card)] p-4"><p className="text-sm font-semibold">Storyboard</p>{project.scenes.length === 0 ? <p className="text-xs text-[var(--text-muted)]">Chưa có cảnh.</p> : project.scenes.map((scene) => <article key={scene.id} className="border-t border-[var(--border)] py-3 first:border-0"><div className="flex items-center justify-between gap-3"><p className="font-semibold">{scene.position + 1}. {scene.title}</p><Status value={scene.status} /></div><p className="mt-1 text-xs text-[var(--text-muted)]">{scene.durationSec}s · {SCENE_KIND_LABEL[scene.kind] || scene.kind}</p><p className="mt-2 whitespace-pre-wrap text-sm text-[var(--text-secondary)]">{scene.script}</p></article>)}</div><aside className="space-y-3 rounded-lg border border-[var(--border)] bg-[var(--bg-card)] p-4"><MediaStatusBadge status={project.mock ? "mock" : project.renderFresh ? project.status : "stale"} /><p className="text-xs text-[var(--text-muted)]">QA <strong className="text-[var(--text)]">{project.qualityScore ?? "Chưa kiểm tra"}</strong></p><p className="text-xs text-[var(--text-muted)]">Approval <strong className="text-[var(--text)]">{project.approvalFresh ? "Đúng revision" : "Chưa có hoặc đã cũ"}</strong></p><p className="rounded-md bg-[var(--bg-subtle)] p-3 text-xs text-[var(--text-muted)]">Chế độ chỉ xem không cho phép sửa cảnh, render hoặc duyệt.</p></aside></div></div>;
}

function VideoProjectCards({ projects, onSelect, emptyText }: { projects: ProjectSummary[]; onSelect: (id: string) => void; emptyText: string }) {
  if (projects.length === 0) return <div className="flex min-h-48 items-center justify-center rounded-lg border border-[var(--border)] text-sm text-[var(--text-muted)]">{emptyText}</div>;
...
function ProviderReadiness({ providers }: { providers: ProviderStatus | null }) {
  ...
  return <div className="max-w-3xl space-y-5">
    <div className="flex flex-wrap items-start justify-between gap-4">
      ...
      <Link href="/system/settings?view=video&scope=account" className="inline-flex min-h-11 items-center justify-center rounded-md bg-[var(--accent)] px-4 text-sm font-semibold text-[var(--accent-foreground)]">Mở cài đặt video</Link>
    </div>
    {!providers ? <div className="flex min-h-48 items-center justify-center"><Spinner size={24} className="animate-spin" /></div> : <>
      <div className="rounded-md border p-4" style={{ borderColor: "var(--border)", background: "var(--bg-subtle)" }}>
