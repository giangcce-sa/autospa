...
export function BrandKitManager() {
  const [form, setForm] = useState<BrandKit>({ logoUrl: "", primaryColor: "#2d6a4f", accentColor: "#40c074", fontStyle: "elegant", spaName: "", tagline: "" });
  const [saved, setSaved] = useState(false);
  const [loading, setLoading] = useState(false);

  useEffect(() => {
    fetch("/api/brand-kit").then((r) => r.json()).then((res) => {
      if (res.data) setForm({ logoUrl: res.data.logoUrl ?? "", primaryColor: res.data.primaryColor, accentColor: res.data.accentColor, fontStyle: res.data.fontStyle, spaName: res.data.spaName ?? "", tagline: res.data.tagline ?? "" });
    });
  }, []);
  ...
  const handleSave = async () => {
    setLoading(true);
    try {
      await fetch("/api/brand-kit", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(form) });
      setSaved(true);
