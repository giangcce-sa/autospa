...
export function StyleTraining() {
  ...
  const [fbConfigured, setFbConfigured] = useState<{ hasToken: boolean; pageId: string; pageName: string } | null>(null);

  const load = () =>
    fetch("/api/style-training").then((r) => r.json()).then((res) => {
  ...
  useEffect(() => {
    ...
    fetch("/api/settings").then((r) => r.json()).then((res) => {
      if (res.data) setFbConfigured({ hasToken: !!res.data.fbAccessToken, pageId: res.data.fbPageId ?? "", pageName: res.data.fbPageName ?? "" });
    });
  }, []);
  ...
  const handleAdd = async () => {
    ...
    try {
      await fetch("/api/style-training", {
        ...
        body: JSON.stringify({ action: "add-sample", content: form.content, likes: +form.likes || 0, comments: +form.comments || 0, shares: +form.shares || 0 }),
      });
  ...
  const handleAnalyze = async () => {
    ...
    setError("");
    try {
      const res = await fetch("/api/style-training", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "analyze" }) });
  ...
  const handleBulkPaste = async () => {
    ...
    // Split by "---" separator or double newlines (user can paste multiple posts)
        ...
        {tab === "facebook" && (
          <Card>
            ...
            <div className="space-y-3">
              ...
              {scanSource === "own" && fbConfigured && (
                <div className="p-2.5 rounded-lg text-xs space-y-1" style={{ background: fbConfigured.hasToken ? "var(--accent-light)" : "var(--rose-light)", border: `1px solid ${fbConfigured.hasToken ? "var(--accent)" : "var(--rose)"}` }}>
                  {fbConfigured.hasToken ? (
                    <>
                      <p style={{ color: "var(--accent)" }}>
                        ✓ Token đã cấu hình{fbConfigured.pageName ? ` — ${fbConfigured.pageName}` : ""}
                        {fbConfigured.pageId ? ` (${fbConfigured.pageId})` : ""}
                      </p>
                      <p style={{ color: "var(--text-muted)" }}>Token từ Graph API Explorer chỉ có hiệu lực ~1 giờ. Nếu bị lỗi, vào Cài đặt → paste token mới → Lưu.</p>
                    </>
                  ) : (
                    <p style={{ color: "var(--rose)" }}>
                      Chưa cấu hình Facebook Token. Vào <strong>Cài đặt → Facebook Page</strong> để thêm token.
                    </p>
                  )}
                </div>
              )}
              ...
              {/* Permission error — detailed guidance */}
              {scanError === "permission" && (
                <div className="rounded-lg overflow-hidden text-xs" style={{ border: "1px solid var(--amber)", background: "var(--amber-light)" }}>
                  <div className="px-3 py-2 font-medium" style={{ color: "var(--amber)" }}>
                    Token thiếu quyền <code>pages_read_engagement</code>
                  </div>
                  <div className="px-3 pb-3 space-y-2" style={{ color: "var(--text-secondary)" }}>
                    ...
                    <button onClick={() => setFbMode("paste")}
                      className="mt-1 underline" style={{ color: "var(--accent)" }}>
                      Hoặc dùng Paste thủ công thay thế →
                    </button>
                  </div>
                </div>
              )}
              {scanError === "invalid" && (
                <div className="rounded-lg overflow-hidden text-xs" style={{ border: "1px solid var(--rose)", background: "var(--rose-light)" }}>
                  ...
                  <div className="px-3 pb-3 space-y-2" style={{ color: "var(--text-secondary)" }}>
                    <p>Token từ Graph API Explorer chỉ dùng được ~1 giờ và phải lưu vào Cài đặt mỗi lần dùng. Để lấy token mới:</p>
                    <ol className="list-decimal list-inside space-y-1">
                      <li>Vào <strong>developers.facebook.com/tools/explorer</strong></li>
                      <li>Chọn <strong>Page Access Token</strong> — chọn đúng trang của bạn</li>
                      ...
                      <li>Quay lại đây và quét lại</li>
                    </ol>
                    <div className="p-2 rounded mt-1" style={{ background: "var(--bg-card)", color: "var(--text-muted)" }}>
                      <p className="font-medium mb-0.5" style={{ color: "var(--text)" }}>Tip: Lấy token 60 ngày</p>
                      <p>Dùng Business Manager → Cài đặt → Tích hợp API → tạo System User token (không hết hạn) để không cần đổi hàng giờ.</p>
                    </div>
                    <button onClick={() => setFbMode("paste")} className="underline" style={{ color: "var(--accent)" }}>
                      Hoặc dùng Paste thủ công (không cần token) →
                    </button>
                  </div>
                </div>
              )}
              {scanError === "other" && (
                <div className="p-2 rounded-lg text-xs" style={{ background: "var(--rose-light)", color: "var(--rose)" }}>
                  {scanErrorMsg}
                </div>
              )}
            </div>
              ...
              <div className="mt-4 space-y-2">
                <div className="flex items-center justify-between">
                  <p className="text-xs font-medium" style={{ color: "var(--text)" }}>
                    Tìm thấy {fetchedPosts.length} bài — đã chọn {selected.size}
                  </p>
